...
- `category`：計畫種類，`research`（國科會等研究計畫，預設）/ `teaching`（教育部教學實踐）/ `industry`（產學合作）。計畫頁依此分區顯示（研究計畫 / 教學計畫 / 產學計畫），空的種類不顯示
- 常用：`grant_id`、`budget_twd`（千元 NTD，中英頁都會顯示）、`active`（`false` 歸「已完成」）、`title_en` / `summary_en`
- 用途：計畫頁（NSTC 研究、教育部教學實踐、產學合作等）
